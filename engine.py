...
- 해시태그 3-4개만 (마지막에)
- **해시태그에 반드시 "#{site_name}" 포함 (다른 매체 이름 사용 금지)**
...
- **해시태그 첫 번째와 두 번째는 반드시 "#{site_name}" "#{site_name_en}"만 사용 (다른 매체 이름 절대 사용 금지)**
- 매체 이름: {site_name} / 영문: {site_name_en}
...
[해시태그 예시: #{site_name} #{site_name_en} #[관련태그1] #[관련태그2] #[관련태그3]]
...
- **해시태그에 반드시 "#{site_name}" 포함 (다른 매체 이름 절대 사용 금지)**
- 매체 이름: {site_name}
- 친구와 대화하듯 편안한 톤
